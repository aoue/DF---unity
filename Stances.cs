...
public class Stances
{
    ...
    //stance trackers : int=duration : -1=not set. 0=last charge/tick. >0=number of charges/ticks remaining
    public int defend { get; set; } //raises pdef/mdef
    public int adrenaline { get; set; } //raises hp and ignores max
    public int shatter { get; set; } //if enemy attack doesn't kill, reduce damage to 1
    public int kindara { get; set; } //raises patk
    public int howl { get; set; } 
    public int growl { get; set; }
    public int mountain { get; set; } //cuts damage in half until hit with a magic attack
    public int suppression { get; set; } //lowers pa/ma

    //stance helpers
    public int adr_loss { get; set; }

    //and DOTs, HOTs
    public int[] bleed = new int[2];
    public int[] poison = new int[2]; //duration, intensity
    public int regen { get; set; } //duration
    ...
    // dots. returns hp after being affected by dots
    ...
    //returns string that lists unit's multipliers, unless multiplier is 1
    ...
    //the trigger functions only purpose is to decrement status effects. that's all.
    //trigger each tick
    public void trigger_tick(PlayerUnit self)
    {
        if (bleed[0] > 0)
        {
            bleed[0]--;
    ...
    public void trigger_attack(PlayerUnit self)
    {
        if (howl > 0)
        {
            howl--;
    ...
    public void trigger_attacked(PlayerUnit self)
    {
        if (mountain == 0)
        {
            exit_mountain();
    ...
    public void trigger_healed(PlayerUnit self)
    {

    }
    //trigger when the unit moves
    public void trigger_move(PlayerUnit self)
    {
    ...
    public int enter_adrenaline(PlayerUnit self, int duration)
    {
        ...
        {
            adr_loss = 0;
            adr_loss = (self.get_hpMax_actual() / 3) + (10 * self.get_level());
            adrenaline = duration;
            return adr_loss;
            //heal self
        }
    ...
    public void exit_adrenaline(PlayerUnit self)
    {
        if (self.get_hp() > self.get_hpMax_actual())
        {
            self.modify_hp_attack(Math.Max(self.get_hpMax_actual(), self.get_hp() - adr_loss));
            //show damage text
        }
        adrenaline = -1;
    }
